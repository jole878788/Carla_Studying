'''
初始化: 配置CARLA连接、车辆、传感器、渲染设置等
动作空间: 支持连续控制([-1,1]转向和[0,1]油门)或离散动作(4种预定义操作)
状态表示: 可配置不同的状态信息组合(waypoints、角度、操作类型等)
奖励系统: 支持自定义奖励函数
'''
...
class CarlaRouteEnv(gym.Env):
    ...
    def __init__(self, host="127.0.0.1", port=2000,
        ...
        #定义动作空间，连续动作空间为[-1, 1]转向和[0, 1]油门，离散动作空间为4种定义操作
    ...
    def reset(self, is_training=False, seed=None, options=None):
